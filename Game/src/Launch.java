...
import com.ayman.jengen.gameCreation.Jengen;
...
public class Launch {
    static GUIContext guiContext = new GUIContext.Builder()
            ...
            .setHeight(300 * 8 / 12)
            .setScaleFactor(3)
            .setMiniMapAlpha(150) // Optional (determines the transparency of the minimap)
...
## Level Creation
 - Jengen provides a simple API for creating and loading levels from files. It supports different types of levels like:
    - Spawn Level: Used to create a level from predefined images or txt files. It first reads the tiles file (txt or png) and then reads the mobs, coins, and effects files (txt) from the level folder.
    - Random Level:
    ...
    - The level is rendered by iterating over the tiles, mobs, coins, and effects and rendering them on the screen.
